...
import { dateKey, formatAxisTime, formatClock, formatLongDate, formatShortDate, formatSqliteDateTime, hoursBetween, minutesBetween, parseSqliteDateTime } from '@/utils/dateTime';
...
function createBins(points: HeartRateRecord[], binCount: number): TrendPoint[] {
  if (points.length === 0) {
    return [];
  }
  ...
  return Array.from({ length: binCount }, (_, index) => {
    const bucketStart = start + size * index;
    const bucketEnd = index === binCount - 1 ? end + 1 : bucketStart + size;
    const bucket = points.filter((point) => {
      const time = point.date.getTime();
      return time >= bucketStart && time < bucketEnd;
    });
    const value = bucket.length === 0 ? points[Math.min(index, points.length - 1)].bpm : Math.round(mean(bucket.map((point) => point.bpm)));
    return {
...
function sleepNeedHours(previousSleeps: SleepCycleRecord[]): number {
  const weights = [0.6, 0.3, 0.1];
  const additional = previousSleeps
    .slice(-3)
    .reverse()
    .map((sleep, index) => {
      const shortfall = Math.max(0, 8 - hoursBetween(sleep.start, sleep.end));
      return shortfall * (weights[index] ?? 0);
    })
    .reduce((sum, value) => sum + value, 0);

  return 8 + Math.min(2.5, additional);
}
...
function scoreSleepCycles(sleeps: SleepCycleRecord[], naps: ActivityRecord[]): SleepCycleRecord[] {
  ...
  for (const sleep of sleeps) {
    ...
    const need = sleepNeedHours(scored);
    const effectiveSleep = hoursBetween(sleep.start, sleep.end) + napCreditHours(recentNaps);
    scored.push({
      ...sleep,
      score: clamp((effectiveSleep / need) * 100, 0, 100),
    });
  }
...
function personalizeRestingHr(sleeps: SleepCycleRecord[], rows: HeartRateRecord[]): number {
  ...
  const dailyMinima = groupByDay(rows).map(([, items]) => Math.min(...items.map((item) => item.bpm)));
  const dailyMedian = median(dailyMinima);
  return dailyMedian === null ? 50 : Math.round(dailyMedian);
}

function personalizeMaxHr(rows: HeartRateRecord[], restingHr: number): number {
  const observed = rows.length === 0 ? 175 : Math.max(...rows.map((row) => row.bpm)) + 5;
...
async function shouldRefreshDerivedData(db: SQLiteDatabase): Promise<boolean> {
  const counts = await db.getFirstAsync<{
    ...
    SELECT
      ...
      (SELECT COUNT(*) FROM sleep_cycles) AS sleep_count,
      (SELECT COUNT(*) FROM activities) AS activity_count,
      (SELECT COUNT(*) FROM sleep_stage_segments) AS stage_count
  `);
  ...
  return counts.sleep_count === 0 || counts.activity_count === 0 || counts.stage_count === 0;
}
...
export class SQLiteHealthRepository implements HealthRepository {
  ...
  async getDashboardSnapshot(): Promise<DashboardSnapshot> {
    ...
    const data = await loadPreparedData(this.db);
    const now = new Date();
    const latestSleep = data.sleepCycles.at(-1) ?? null;
    const restingHr = personalizeRestingHr(data.sleepCycles, data.heartRows);
    const maxHr = personalizeMaxHr(data.heartRows, restingHr);
    const recovery = estimateRecoveryScore(latestSleep, data.sleepCycles, data.heartRows);

    const last24Hours = data.heartRows.filter((row) => now.getTime() - row.date.getTime() <= 24 * 3600000);
    const intradayRows = last24Hours.length > 0 ? last24Hours : data.heartRows;
    const heartCardSeries = createBins(intradayRows, 12);
    const latestDayRows = data.heartRows.filter((row) => dateKey(row.date) === dateKey(data.heartRows.at(-1)?.date ?? now));
    const todayStrain = calculateStrain(latestDayRows, maxHr, restingHr);
    const strainSeries = buildDailyTrend('14d', data.heartRows, (rows) => calculateStrain(rows, maxHr, restingHr));
    const stageRecords = latestSleep ? data.sleepStages.filter((stage) => stage.sleepId === latestSleep.sleepId) : [];
    ...
    if (data.heartRows.length === 0) {
      return {
        ...
        lastSyncLabel: data.deviceState?.last_synced_at ?? 'Local seed loaded',
      };
    }
    ...
    return {
      ...
      heartCard: {
        ...
        maxHr: Math.max(...intradayRows.map((row) => row.bpm)),
      ...
      lastSyncLabel: data.deviceState?.last_synced_at ? `Last sync ${formatClock(parseSqliteDateTime(data.deviceState.last_synced_at))}` : 'Local seed loaded',
    };
  }
  ...
  async getSleepHistory(range: HistoryRange): Promise<SleepHistorySnapshot> {
    await this.ensurePrepared();
    const data = await loadPreparedData(this.db);
    const sessions = data.sleepCycles.slice(-rangeDays(range)).reverse();
    ...
    const durationValues = sessions.map((session) => minutesBetween(session.start, session.end));
    ...
    const mappedSessions: SleepSession[] = sessions.map((session) => {
      const stages = aggregateSleepStages(data.sleepStages.filter((stage) => stage.sleepId === session.sleepId));
      return {
        id: session.id,
  ...
  async getHeartHistory(range: HistoryRange): Promise<HeartHistorySnapshot> {
    ...
    const restingHr = personalizeRestingHr(data.sleepCycles, data.heartRows);
    const intradaySource = data.heartRows.filter((row) => {
      const last = data.heartRows.at(-1)?.date ?? new Date();
      return last.getTime() - row.date.getTime() <= 24 * 3600000;
    });
    const grouped = groupByDay(data.heartRows);
    const weeklyResting = grouped.slice(-rangeDays(range)).map(([day, items]) => ({
      label: formatShortDate(parseSqliteDateTime(`${day} 00:00:00`)),
      value: Math.min(...items.map((item) => item.bpm)),
    }));
    ...
    return {
      ...
      maxHr: Math.max(...intradaySource.map((row) => row.bpm)),
      intraday: createBins(intradaySource, 12),
      weeklyResting,
  ...
  async getWellnessSnapshot(range: HistoryRange): Promise<WellnessSnapshot> {
    ...
    const restingHr = personalizeRestingHr(data.sleepCycles, data.heartRows);
    const maxHr = personalizeMaxHr(data.heartRows, restingHr);
    const recoveryTrend = data.sleepCycles.slice(-rangeDays(range)).map((sleep, index, sleeps) => {
      const score = estimateRecoveryScore(sleep, sleeps.slice(0, index + 1), data.heartRows).score;
      return {
    ...
    const stressRows = data.heartRows.filter((row) => row.stress !== null);
    const spo2Rows = data.heartRows.filter((row) => row.spo2 !== null);
    const tempRows = data.heartRows.filter((row) => row.skinTemp !== null);
    ...
    const activities = data.activities
      ...
      .map((activity): ActivitySummary => {
        const rows = rowsInRange(data.heartRows, activity.start, activity.end);
        return {
          id: activity.id,
