...
use crate::{constants::PacketType, error::WearableError, helpers::BufferReader};

#[derive(Debug)]
pub struct WearablePacket {
...
impl WearablePacket {
    ...
    pub fn with_seq(self, seq: u8) -> WearablePacket {
        WearablePacket { seq, ..self }
    }
    ...
    pub fn from_data(mut data: Vec<u8>) -> Result<Self, WearableError> {
        if data.len() < 8 {
            return Err(WearableError::PacketTooShort);
        }

        let sof = data.pop_front()?;
        if sof != Self::SOF {
            return Err(WearableError::InvalidSof);
        }
        ...
        if calculated_crc8 != expected_crc8 {
            return Err(WearableError::InvalidHeaderCrc8);
        }

        let length = usize::from(u16::from_le_bytes(length_buffer));
        let partial = data.len() < length;
        if length < 8 {
            return Err(WearableError::InvalidPacketLength);
        }
        ...
        if !partial {
            ...
            if calculated_crc32 != expected_crc32 {
                return Err(WearableError::InvalidDataCrc32);
            }
        }
        ...
        Ok(Self {
            packet_type: {
                ...
                PacketType::from_u8(packet_type)
                    .ok_or(WearableError::InvalidPacketType(packet_type))?
            },
            seq: data.pop_front()?,
            cmd: data.pop_front()?,
            data,
    ...
    pub fn framed_packet(&self) -> Result<Vec<u8>, WearableError> {
        let pkt = self.create_packet();
        let length = u16::try_from(pkt.len()).map_err(|_| WearableError::Overflow)? + 4;
...
impl fmt::Display for WearablePacket {
...
mod tests {
    ...
    fn test_packet_creation() {
        let packet = WearablePacket::new(PacketType::Command, 1, 5, vec![0x01, 0x02, 0x03]);
        let framed = packet.framed_packet().unwrap();
        assert!(framed.len() > 8);
        assert_eq!(framed[0], WearablePacket::SOF);
    }
    ...
    #[test]
    fn packet_too_short() {
        let result = WearablePacket::from_data(vec![0xAA, 0x01]);
        assert!(matches!(result, Err(WearableError::PacketTooShort)));
    }
    ...
    fn invalid_header_crc8() {
        ...
        let result = WearablePacket::from_data(data);
        assert!(matches!(result, Err(WearableError::InvalidHeaderCrc8)));
    }

    #[test]
    fn with_seq_changes_seq() {
        let packet = WearablePacket::new(PacketType::Command, 0, 1, vec![]);
    ...
    #[test]
    fn display_format() {
        let packet = WearablePacket::new(PacketType::Command, 1, 5, vec![0xAB, 0xCD]);
    ...
    fn roundtrip_all_packet_types() {
        for pt in [
            ...
            let original = WearablePacket::new(pt, 7, 3, vec![0x01, 0x02]);
    ...
    fn empty_payload_creates_valid_frame() {
        let packet = WearablePacket::new(PacketType::Command, 0, 0, vec![]);
        let framed = packet.framed_packet().unwrap();
