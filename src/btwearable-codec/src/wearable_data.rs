...
impl WearableData {
    pub fn from_packet(packet: WearablePacket) -> Result<Self, WearableError> {
        match packet.packet_type {
            ...
            PacketType::CommandResponse => {
                let command = CommandNumber::from_u8(packet.cmd)
                    .ok_or(WearableError::InvalidCommandType(packet.cmd))?;
    ...
    fn parse_realtime_hr(packet: WearablePacket) -> Result<Self, WearableError> {
        let ts0 = packet.cmd;
    ...
    fn parse_console_log(mut packet: Vec<u8>) -> Result<Self, WearableError> {
        ...
        result.extend(iter);
        // not sure why this happens but sometimes Wearable gives logs
    ...
    fn parse_historical_packet(version: u8, packet: Vec<u8>) -> Result<Self, WearableError> {
    ...
    fn parse_historical_packet_generic(mut packet: Vec<u8>) -> Result<Self, WearableError> {
        ...
        if rr.len() != rr_count {
            return Err(WearableError::InvalidRRCount);
        }
    ...
    fn parse_historical_packet_v12(data: Vec<u8>) -> Result<Self, WearableError> {
        if data.len() < 77 {
        ...
        let unix = u64::from(u32::from_le_bytes(
            d[4..8].try_into().map_err(|_| WearableError::InvalidData)?,
        )) * 1000;
        ...
        for i in 0..rr_count.min(4) {
            ...
            let val = u16::from_le_bytes(
                d[off..off + 2]
                    .try_into()
                    .map_err(|_| WearableError::InvalidData)?,
            );
            if val != 0 {
                rr.push(val);
            }
    ...
    fn parse_historical_packet_with_imu(mut packet: Vec<u8>) -> Result<Self, WearableError> {
        ...
        let read_imu_axis_data =
            |packet_data: &[u8], offset: usize| -> Result<Vec<i16>, WearableError> {
    ...
    fn parse_report_version_info(mut data: Vec<u8>) -> Result<Self, WearableError> {
        let _ = data.read::<3>();
    ...
    fn parse_alarm_time_response(mut data: Vec<u8>) -> Result<Self, WearableError> {
...
mod tests {
    ...
    fn parse_historical_packet() {
        ...
        let packet = WearablePacket::from_data(data).expect("Invalid packet data");
        let data = WearableData::from_packet(packet).expect("Invalid packet");
        assert_eq!(
            data,
            WearableData::HistoryReading(HistoryReading {
                unix: 1748326124000,
                bpm: 62,
                rr: vec![837],
                imu_data: [
    ...
    fn parse_console_logs() {
        ...
        let data = WearableData::from_packet(packet).expect("Invalid data");
        assert_eq!(
            data,
            WearableData::ConsoleLog {
                unix: 1735199614,
    ...
    fn parse_event() {
        ...
        assert_eq!(data, WearableData::RunAlarm { unix: 1733561527 });
    }

    #[test]
    fn parse_realtime_hr() {
        let packet = WearablePacket {
            packet_type: PacketType::RealtimeData,
            seq: 0,
            cmd: 0x78,
            data: vec![0x56, 0x34, 0x12, 0x34, 0x12, 72],
            size: 0,
            partial: false,
        };

        let data = WearableData::from_packet(packet).expect("Invalid realtime data");
        assert_eq!(
            data,
            WearableData::RealtimeHr {
                unix: 0x12345678,
                bpm: 72,
            }
        );
    ...
    fn parse_version_response() {
        ...
        let packet = WearablePacket::from_data(response).expect("invalid packet");
        let data = WearableData::from_packet(packet).expect("invalid packet");
        assert_eq!(
            data,
            WearableData::VersionInfo {
