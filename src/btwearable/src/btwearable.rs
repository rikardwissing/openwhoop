...
impl BtWearable {
        ...
        let data = match packet.uuid {
            DATA_FROM_STRAP => {
                let packet = if let Some(mut wearable_packet) = self.packet.take() {
                    ...
                    wearable_packet.data.extend_from_slice(&packet.bytes);

                    if wearable_packet.data.len() + 3 >= wearable_packet.size {
                        wearable_packet
                    } else {
                        self.packet = Some(wearable_packet);
                        return Ok(None);
                    }
                } else {
                    let packet = WearablePacket::from_data(packet.bytes)?;
                    if packet.partial {
                ...
                let Ok(data) = WearableData::from_packet(packet) else {
                    return Ok(None);
                };
                data
            }
    ...
    async fn handle_data(&mut self, data: WearableData) -> anyhow::Result<Option<WearablePacket>> {
        match data {
            ...
            WearableData::RunAlarm { .. } => {}
            WearableData::AlarmInfo { .. } => {}
            WearableData::Event { .. } => {}
            WearableData::VersionInfo { harvard, boylston } => {
...
fn map_sleep_cycle(sleep: btwearable_entities::sleep_cycles::Model) -> SleepCycle {
    SleepCycle {
        id: sleep.end.date(),
        start: sleep.start,
        end: sleep.end,
